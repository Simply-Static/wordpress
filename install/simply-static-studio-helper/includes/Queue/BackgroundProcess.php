...
abstract class BackgroundProcess extends AsyncRequest {
	...
	public function dispatch() {
		...
		Logger::log('Scheduling Event');
		// Schedule the cron healthcheck.
		$this->schedule_event();
		Logger::log('Dispatching original method');
		// Perform remote post.
	...
	public function test_handle() {
		...
		if ( $this->is_cancelled() ) {
			$this->clear_scheduled_event();
			$this->delete_all();
			echo "Cancelled and all deleted";
			return die();
		}
	...
	public function handle_cron_healthcheck() {
		Logger::log('Healtcheck in Migration prcess');
		if ( $this->is_processing() ) {
			// Background process already running.
			exit;
		}
		Logger::log('Process not processing yet, going to dispatch');

		if ( $this->is_queue_empty() ) {
			Logger::log('Migration queue empty.');
			// No data to process.
			$this->clear_scheduled_event();
			exit;
		}

		Logger::log('Migration event dispatching');
		$this->dispatch();
	}
