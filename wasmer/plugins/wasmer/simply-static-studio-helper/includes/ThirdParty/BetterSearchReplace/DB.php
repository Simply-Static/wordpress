...
namespace Simply_Static_Studio\ThirdParty\BetterSearchReplace;
...
class DB {
	...
	public function get_page_size() {
		$page_size = get_option( 'sss_bsr_page_size', false ) ? get_option( 'sss_bsr_page_size' ) : 1000;
				...
				if ( $this->wpdb->options === $table ) {
					...
					if ( isset( $update_later ) && true === $update_later ) {
						...
						if ( $edited_data != $data_to_fix ) {
							...
							update_option( 'sss_bsr_update_site_url', $edited_data );
							continue;
						}
					}
	 ...
	 * @return boolean
	 */
	public function maybe_update_site_url( $prefix ) {
		$option = get_option( 'sss_bsr_update_site_url' );

		if ( $option ) {
			global $wpdb;
			$wpdb->query( $wpdb->prepare( "UPDATE {$prefix}options SET option_value = %s WHERE option_name = 'siteurl'", $option ) );
			$wpdb->query( $wpdb->prepare( "UPDATE {$prefix}options SET option_value = %s WHERE option_name = 'home'", $option ) );

			delete_option( 'sss_bsr_update_site_url' );
			return true;
		}
