 ...
 * ╚══════╝╚══════╝╚═╝  ╚═╝  ╚═══╝  ╚═╝     ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
 */
namespace Simply_Static_Studio\ThirdParty\Servmask\database;
...
function ai1wm_open( $file, $mode ) {
	...
	if ( false === $file_handle ) {
		throw new Ai1wm_Not_Accessible_Exception( sprintf( __( 'Could not open %s with mode %s. The process cannot continue.', 'simply-static-studio' ), $file, $mode ) );
	}
...
function ai1wm_write( $handle, $content ) {
	...
	if ( false === $write_result ) {
		if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
			throw new Ai1wm_Not_Writable_Exception( sprintf( __( 'Could not write to: %s. The process cannot continue.', 'simply-static-studio' ), $meta['uri'] ) );
		}
	} elseif ( null === $write_result ) {
		return strlen( $content );
	} elseif ( strlen( $content ) !== $write_result ) {
		if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
			throw new Ai1wm_Quota_Exceeded_Exception( sprintf( __( 'Out of disk space. Could not write to: %s. The process cannot continue.', 'simply-static-studio' ), $meta['uri'] ) );
		}
	}
...
function ai1wm_read( $handle, $length ) {
	if ( $length > 0 ) {
		...
		if ( false === $read_result ) {
			if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
				throw new Ai1wm_Not_Readable_Exception( sprintf( __( 'Could not read file: %s. The process cannot continue.', 'simply-static-studio' ), $meta['uri'] ) );
			}
		}
...
function ai1wm_seek( $handle, $offset, $mode = SEEK_SET ) {
	...
	if ( -1 === $seek_result ) {
		if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
			throw new Ai1wm_Not_Seekable_Exception( sprintf( __( 'Could not seek to offset %d on %s. The process cannot continue.' , 'simply-static-studio' ), $offset, $meta['uri'] ) );
		}
	}
...
function ai1wm_tell( $handle ) {
	...
	if ( false === $tell_result ) {
		if ( ( $meta = stream_get_meta_data( $handle ) ) ) {
			throw new Ai1wm_Not_Tellable_Exception( sprintf( __( 'Could not get current pointer position of %s. The process cannot continue.', 'simply-static-studio'), $meta['uri'] ) );
		}
	}
...
function ai1wm_verify_secret_key( $secret_key ) {
	if ( $secret_key !== get_option( AI1WM_SECRET_KEY ) ) {
		throw new Ai1wm_Not_Valid_Secret_Key_Exception( __( 'Could not authenticate the secret key. The process cannot continue.', 'simply-static-studio' ) );
	}
...
function ai1wm_encrypt_string( $string, $key ) {
	...
	if ( $iv === false ) {
		throw new Ai1wm_Not_Encryptable_Exception( __( 'Could not generate random bytes. The process cannot continue.', 'simply-static-studio' ) );
	}

	$encrypted_string = openssl_encrypt( $string, AI1WM_CIPHER_NAME, $key, OPENSSL_RAW_DATA, $iv );
	if ( $encrypted_string === false ) {
		throw new Ai1wm_Not_Encryptable_Exception( __( 'Could not encrypt data. The process cannot continue.', 'simply-static-studio' ) );
	}
...
function ai1wm_crypt_iv_length() {
	...
	if ( $iv_length === false ) {
		throw new Ai1wm_Not_Encryptable_Exception( __( 'Could not obtain cipher length. The process cannot continue.', 'simply-static-studio' ) );
	}
...
function ai1wm_decrypt_string( $encrypted_string, $key ) {
	...
	if ( $decrypted_string === false ) {
		throw new Ai1wm_Not_Decryptable_Exception( __( 'Could not decrypt data. The process cannot continue.', 'simply-static-studio' ) );
	}
